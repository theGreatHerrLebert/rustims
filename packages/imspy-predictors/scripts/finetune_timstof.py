#!/usr/bin/env python
"""
Fine-tune pre-trained model on timsTOF MS2 dataset.

This script loads a pre-trained encoder from PROSPECT pre-training and fine-tunes
it on the smaller timsTOF-specific dataset.

Usage:
    python scripts/finetune_timstof.py \
        --pretrained ./checkpoints/pretrain_prospect/pretrained_encoder.pt \
        --output-dir ./checkpoints/timstof_finetuned
"""
...
def main():
    ...
    parser.add_argument("--freeze-encoder", action="store_true",
                        help="Freeze encoder and only train head")
    parser.add_argument("--output-dir", type=str, default="./checkpoints/timstof_finetuned")
    ...
    logger.info(f"Pre-trained model: {args.pretrained}")
    ...
    # Load datasets
    logger.info("Loading timsTOF datasets...")
    ...
    pretrain_val_loss = checkpoint.get('val_loss', 'N/A')
    logger.info(f"Pre-training validation loss was: {pretrain_val_loss}")
    ...
    baseline_loss = validate(model, val_loader, device, use_amp)
    logger.info(f"Baseline val loss (pre-trained): {baseline_loss:.4f}")
    ...
    for epoch in range(args.epochs):
        ...
        if val_loss < best_val_loss:
            ...
            logger.info(f"New best model! (val_loss: {val_loss:.4f}, improvement: {improvement:.4f})")
        else:
            patience_counter += 1
    ...
    logger.info(f"Improvement from baseline: {baseline_loss - final_val_loss:.4f}")

    # Save final model
    model.save_pretrained(str(output_dir / "finetuned_model.pt"))
    logger.info(f"Fine-tuning complete! Best val loss: {best_val_loss:.4f}")

    # Summary
    logger.info("\n=== Summary ===")
    logger.info(f"Pre-train val loss:  {pretrain_val_loss}")
    logger.info(f"Baseline val loss:   {baseline_loss:.4f}")
    logger.info(f"Fine-tuned val loss: {best_val_loss:.4f}")
