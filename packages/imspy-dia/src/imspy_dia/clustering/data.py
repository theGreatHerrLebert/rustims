"""
Core data structures for DIA clustering.

This module provides Python wrappers around Rust clustering data structures.
"""
from __future__ import annotations

from typing import Iterator, Dict, List, Optional, Tuple, Union, Any
import numpy as np
import pandas as pd

from imspy_core import RustWrapperObject
...
class Fit1D(RustWrapperObject):
    ...
    @classmethod
    def from_py_ptr(cls, p) -> "Fit1D":
        inst = cls.__new__(cls)
        inst._Fit1D__py_ptr = p
        return inst
...
class RtPeak1D(RustWrapperObject):
    ...
    @classmethod
    def from_py_ptr(cls, p) -> "RtPeak1D":
        inst = cls.__new__(cls)
        inst._RtPeak1D__py_ptr = p
        return inst
...
class ImPeak1D(RustWrapperObject):
    ...
    @classmethod
    def from_py_ptr(cls, p) -> "ImPeak1D":
        inst = cls.__new__(cls)
        inst._ImPeak1D__py_ptr = p
        return inst
    ...
    def from_detected(
        ...
        """Build a single ImPeak1D from detector output."""
    ...
    def batch_from_detected(
        ...
        """Vectorized path: call into Rust batch constructor."""
...
class TofScanWindowGrid(RustWrapperObject):
    ...
    @classmethod
    def from_py_ptr(cls, p) -> "TofScanWindowGrid":
        inst = cls.__new__(cls)
        inst._TofScanWindowGrid__py_ptr = p
        return inst
...
class TofScanPlan(RustWrapperObject):
    ...
    def __init__(self, *args, **kwargs):
        raise RuntimeError("Use TofScanPlan.from_py_ptr(...) or TimsDatasetDIAClustering.plan_tof_scan_windows(...)")

    @classmethod
    def from_py_ptr(cls, p) -> "TofScanPlan":
        inst = cls.__new__(cls)
        inst._TofScanPlan__py_ptr = p
        return inst
...
class TofScanPlanGroup(RustWrapperObject):
    ...
    def __init__(self, *args, **kwargs):
        raise RuntimeError("Use TofScanPlanGroup.from_py_ptr(...) or TimsDatasetDIAClustering.plan_tof_scan_windows_for_group(...)")

    @classmethod
    def from_py_ptr(cls, p) -> "TofScanPlanGroup":
        inst = cls.__new__(cls)
        inst._TofScanPlanGroup__py_ptr = p
        return inst
...
class ClusterResult1D:
    ...
    def rt_mu(self) -> float:
        return float(self.rt_fit.mu)

    @property
    def rt_sigma(self) -> float:
        return float(self.rt_fit.sigma)

    @property
    def rt_height(self) -> float:
        return float(self.rt_fit.height)

    @property
    def rt_area(self) -> float:
        return float(self.rt_fit.area)

    @property
    def im_mu(self) -> float:
        return float(self.im_fit.mu)

    @property
    def im_sigma(self) -> float:
        return float(self.im_fit.sigma)

    @property
    def im_height(self) -> float:
        return float(self.im_fit.height)

    @property
    def im_area(self) -> float:
        return float(self.im_fit.area)

    @property
    def tof_mu(self) -> float:
        return float(self.tof_fit.mu)

    @property
    def tof_sigma(self) -> float:
        return float(self.tof_fit.sigma)

    @property
    def tof_height(self) -> float:
        return float(self.tof_fit.height)

    @property
    def tof_area(self) -> float:
        return float(self.tof_fit.area)
    ...
    @classmethod
    def from_py_ptr(cls, p) -> "ClusterResult1D":
        return cls(p)
...
class TofRtGrid(RustWrapperObject):
    ...
    def __init__(self, *args, **kwargs):
        raise RuntimeError(
            "Use TofRtGrid.from_py_ptr(...) or TimsDatasetDIAClustering.tof_rt_grid_* methods."
        )

    @classmethod
    def from_py_ptr(cls, p) -> "TofRtGrid":
        inst = cls.__new__(cls)
        inst._TofRtGrid__py_ptr = p
        return inst
    ...
    def __getitem__(self, key: Union[Index, tuple[Index, Index]]) -> np.ndarray:
