...
def _prefetch_wg_data(wg):
    """Prefetch window grid data in background thread."""
    return wg.data
...
def _detect_im_peaks_for_wgs(
    ...
    *,
    ...
    topk_per_tile: int | None = None,
    patch_batch_target_mb: int = 128,
    prefetch: bool = True,       # NEW: enable data prefetching
):
    from imspy_dia.clustering.data import ImPeak1D

    batch_objs = []
    wgs_list = list(wgs)  # materialize for lookahead

    # Use prefetch executor for overlapping data load with GPU compute
    with ThreadPoolExecutor(max_workers=1) as prefetch_exec:
        future_data = None

        for wg_idx, wg in enumerate(
            tqdm(wgs_list, desc="Detecting peaks (BATCH)", leave=False, ncols=80)
        ):
            # Get current data (from prefetch future or load now)
            with _log_timing(f"im.wg[{wg_idx}].load_data"):
                if prefetch and future_data is not None:
                    B = future_data.result()
                else:
                    B = wg.data

            # Start prefetching next window's data while we process current
            if prefetch and wg_idx + 1 < len(wgs_list):
                future_data = prefetch_exec.submit(_prefetch_wg_data, wgs_list[wg_idx + 1])
            else:
                future_data = None
...
def detect_rt_peaks_for_grid(
    ...
    *,
    ...
    """
    from imspy_dia.clustering.data import RtPeak1D
