"""
timsTOF cluster summary report generator.

Generates multi-page PDF and JSON summary from cluster data.
"""
...
def build_cluster_report(
    ...
    if not HAS_MATPLOTLIB:
        raise ImportError("matplotlib is required for cluster reports. Install with: pip install matplotlib")

    need_cols = set(NUMERIC_COLS + BOOL_COLS + CATEGORICAL_COLS)
    ensure_columns(df, [c for c in need_cols if c in df.columns])
    ...
    with PdfPages(out_pdf_path) as pdf:
        ...
        for xcol, ycol, logx, logy, xl, yl in DENSITY_PAIRS:
            if xcol not in df2d or ycol not in df2d:
                continue
