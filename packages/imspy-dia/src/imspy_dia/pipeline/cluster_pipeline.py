...
from imspy_dia.clustering.torch_extractor import iter_im_peaks_batches
...
def _default_log_path_from_cfg(cfg: dict) -> Path | None:
    ...
    return Path(out_dir) / "logs" / f"open_tracer_{ts}.log"


def main(argv=None):
    parser = argparse.ArgumentParser(
        description="open_tracer: IM peak detection → stitching → clustering (TOML-configured)."
    )
    ...
    parser.add_argument("--log-file", default=None,
                        help="Log file path (defaults to [output]/logs/open_tracer_*.log)")
