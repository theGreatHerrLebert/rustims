"""
DIA dataset with clustering capabilities for timsTOF data.

This module extends the base TimsDatasetDIA with clustering methods.
"""
...
from imspy_core import RustWrapperObject, TimsDatasetDIA, TimsFrame
...
class ScoredHit(RustWrapperObject):
    ...
    def __init__(self, *a, **k):
        raise RuntimeError(
            "Use ScoredHit.from_py_ptr(...) instead of constructing directly."
        )
    ...
    def __repr__(self) -> str:
        return f"ScoredHit(ms2_index={self.ms2_index}, score={self.score:.4f})"
...
class FragmentIndex(RustWrapperObject):
    ...
    Built from a TimsDatasetDIAClustering instance or loaded from parquet.
    """

    def __init__(self, *a, **k):
        raise RuntimeError(
            "Use TimsDatasetDIAClustering.build_fragment_index(...) "
    ...
    def score_features_to_pseudospectra(
        ...
        from imspy_dia.clustering.pseudo import PseudoSpectrum
...
ClusterResult1D = "ClusterResult1D"
SimpleFeature = "SimpleFeature"
PseudoSpectrum = "PseudoSpectrum"


class TimsDatasetDIAClustering(TimsDatasetDIA):
    """
    Extended TimsDatasetDIA with clustering capabilities.

    This class extends the base TimsDatasetDIA from imspy_core with methods
    for hierarchical DIA feature extraction and clustering.
    """
    ...
    def __init__(self, data_path: str, in_memory: bool = False, use_bruker_sdk: bool = True):
        ...
        self._dataset = ims.PyTimsDatasetDIA(self.data_path, self.binary_path, in_memory, self.use_bruker_sdk)

    @classmethod
    def from_py_ptr(cls, obj):
        instance = cls.__new__(cls)
        instance._dataset = obj
        return instance
    ...
    def clusters_for_group(
        ...
        from imspy_dia.clustering.data import ClusterResult1D
        ...
        py_results = self._dataset.clusters_for_group(
            int(window_group),
            int(tof_step),
    ...
    def clusters_for_precursor(
        ...
        py_results = self._dataset.clusters_for_precursor(
            int(tof_step),
            [p.get_py_ptr() for p in im_peaks],
            int(bin_pad),
            float(smooth_sigma_sec),
    ...
    def plan_tof_scan_windows(
        ...
        from imspy_dia.clustering.data import TofScanPlan
    ...
    def plan_tof_scan_windows_for_group(
        ...
        from imspy_dia.clustering.data import TofScanPlanGroup
    ...
    def build_pseudo_spectra_geom(
        ...
        from imspy_dia.clustering.data import PseudoBuildResult
        ...
        inner_res = self._dataset.build_pseudo_spectra_from_clusters_geom(
            ms1_ptrs,
            ms2_ptrs,
            feats_ptrs,
    ...
    def build_pseudo_spectra_xic(
        ...
        inner_res = self._dataset.build_pseudo_spectra_from_clusters_xic(
            ms1_ptrs,
            ms2_ptrs,
            feats_ptrs,
            float(min_rt_jaccard),
    ...
    def build_pseudo_spectra_all_pairs(
        ...
        result = self._dataset.build_pseudo_spectra_all_pairs_from_clusters(
            ms1_ptrs,
            ms2_ptrs,
            feats_ptrs,
            int(top_n_fragments),
    ...
    def window_groups_for_precursor(self, prec_mz: float, im_apex: float) -> List[int]:
        ...
        """
        return self._dataset.window_groups_for_precursor(float(prec_mz), float(im_apex))

    def tof_rt_grid_precursor(self, tof_step: int = 1) -> "TofRtGrid":
        from imspy_dia.clustering.data import TofRtGrid
        grid = self._dataset.tof_rt_grid_precursor(int(tof_step))
        return TofRtGrid.from_py_ptr(grid)

    def tof_rt_grid_for_group(self, window_group: int, tof_step: int = 1) -> "TofRtGrid":
        from imspy_dia.clustering.data import TofRtGrid
        grid = self._dataset.tof_rt_grid_for_group(int(window_group), int(tof_step))
        return TofRtGrid.from_py_ptr(grid)
    ...
    def debug_extract_raw_for_clusters(
        ...
        from imspy_dia.clustering.data import RawPoints
...
def save_clusters_bin(
    ...
    *,
    ...
    if atomic:
        ...
        try:
            ims.save_clusters_bin(str(tmp_path), rust_clusters, bool(compress), bool(strip_points), bool(strip_axes))
        ...
        ims.save_clusters_bin(str(path), rust_clusters, bool(compress), bool(strip_points), bool(strip_axes))
...
def save_clusters_parquet(
    ...
    *,
    ...
    if atomic:
        ...
        with tempfile.NamedTemporaryFile(prefix=".tmp_", suffix=_PARQUET_SUFFIX, dir=tmp_dir, delete=False) as tmp:
            tmp_path = Path(tmp.name)

        try:
            ims.save_clusters_parquet(str(tmp_path), rust_clusters, bool(strip_points), bool(strip_axes))
        ...
        ims.save_clusters_parquet(str(p), rust_clusters, bool(strip_points), bool(strip_axes))
...
def save_pseudo_spectra_bin(
    ...
    *,
    ...
    rust_spectra = [s._p for s in spectra]
    _ensure_dir(p)
