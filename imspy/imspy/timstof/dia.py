...
class TimsDatasetDIA(TimsDataset, RustWrapperObject):
    ...
    def clusters_from_im_peaks_conditioned(
            self,
            rt_index: "RtIndex",
            im_peaks: list["ImPeak1D"],
            *,
            k_rt_sigma: float | None = 3.0,
            max_rt_span_frames: int | None = 200,
            mz_ppm_window: float = 10.0,
            mz_hist_bins: int = 64,
            eval_opts: "EvalOptions | None" = None,
            num_threads: int = 4,
    ):
        """
        IM-first → RT-conditioned: build ClusterSpec from IM peaks conditioned on (IM ∧ m/z),
        optionally restrict RT to μ±kσ and/or cap RT span, then evaluate clusters.
        ...
        im_peaks_py = [p.get_py_ptr() for p in im_peaks]
        opts_py = eval_opts.get_py_ptr() if eval_opts is not None else None

        specs_py, clusters_py = self.__dataset.clusters_from_im_peaks_conditioned(
            rt_index.get_py_ptr(),
            im_peaks_py,
            k_rt_sigma,
            max_rt_span_frames,
        ...
        from .cluster import ClusterSpec, ClusterResult
        specs = [ClusterSpec.from_py_ptr(s) for s in specs_py]
        clusters = [ClusterResult.from_py_ptr(c) for c in clusters_py]
        return specs, clusters

    def clusters_from_im_peaks_conditioned_for_group(
            self,
            window_group: int,
        ...
        """
        Same as clusters_from_im_peaks_conditioned, but evaluated on a **DIA group** RT index.
        If EvalOptions.window_group_hint is not set, it will be stamped with `window_group`.
        """
        if self.use_bruker_sdk:
            warnings.warn("Using Bruker SDK, forcing num_threads=1.")
            num_threads = 1
        ...
        specs_py, clusters_py = self.__dataset.clusters_from_im_peaks_conditioned_for_group(
            int(window_group),
            rt_index_group.get_py_ptr(),
            im_peaks_py,
            k_rt_sigma,
            max_rt_span_frames,
            float(mz_ppm_window),
