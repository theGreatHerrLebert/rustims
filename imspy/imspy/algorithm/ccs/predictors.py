...
class DeepPeptideIonMobilityApex(PeptideIonMobilityApex):
    ...
    def simulate_ion_mobilities(self,
        ...
        return np.array([1 / ccs_to_one_over_k0(c, m, z) for c, m, z in zip(ccs, mz, charges)])
    ...
    def simulate_ion_mobilities_pandas(self, data: pd.DataFrame, batch_size: int = 1024) -> pd.DataFrame:
        ...
        data[f'mobility_{self.name}'] = np.array([1 / ccs_to_one_over_k0(c, m, z) for c, m, z in zip(ccs, m, data.charge.values)])
