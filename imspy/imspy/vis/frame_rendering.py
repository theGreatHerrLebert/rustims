...
def configure_gpu_memory(limit_gb: float = 4.0):
    ...
    if not gpus:
        return
    cfg = tf.config.experimental.VirtualDeviceConfiguration(
        memory_limit=int(limit_gb * 1024)
    )
    for gpu in gpus:
...
def overlay_windows(ax, df_windows, image_width: int,
    ...
    """
    Overlay PASEF window or fragment boxes.
    Boxes colored by WindowGroup or Frame (DDA).
    Text placement adjustable via offsets and alignment.
    """
    for _, row in df_windows.iterrows():
        key = row.get('WindowGroup', row.get('Frame'))
        color = choose_color(int(key))
        ...
        ax.add_patch(rect)
        lbl = (f"ID:{int(row.get('Precursor', key))} "
               f"CE:{row['CollisionEnergy']:.1f}eV "
               f"m/z:{mz_mid:.2f}±{iso_w:.2f}")
        txt_x = x + text_dx
        txt_y = y + text_dy
        ax.text(txt_x, txt_y, lbl,
...
class BaseFrameRenderer:
    """
    Base class for rendering TIMS frames and building videos.
    Subclasses implement `_render_frame` and provide `_all_frame_ids`.
    """
    ...
    def batch_render(self, out_dir: str, frame_ids=None, **render_kwargs):
        ...
        for fid in tqdm(frame_ids, desc="", ncols=80):
...
class DDAFrameRenderer(BaseFrameRenderer):
    """
    Renderer for DDA-PASEF (.d via TimsDatasetDDA).
    """
    def __init__(self, data_path: str,
        ...
        self.handle = TimsDatasetDDA(data_path,
        ...
        self.fragments_map = {
            f: set(range(f + 1, f + d))
            for f, d in zip(self.handle.precursor_frames[:-1], diffs)
        }
    ...
    def _render_frame(self, frame_id: int, save_path=None,
                      dpi: int = 150, cmap: str = 'viridis',
                      annotate: bool = True):
        F = get_frame_matrix(self.handle, frame_id)
        fig, ax = plt.subplots(
            figsize=(10 * F.shape[1] / F.shape[0], 10), dpi=dpi
        )
        ...
        if annotate:
            if frame_id in self.precursor_frames:
                dfp = self.meta[self.meta['Frame'].isin(self.fragments_map.get(frame_id, []))]
                dfp = dfp.rename(columns={'Frame': 'WindowGroup'})
                overlay_windows(ax, dfp, F.shape[1])
            if frame_id in self.fragment_frames:
                dff = self.meta[self.meta['Frame'] == frame_id]
                dff = dff.rename(columns={'Frame': 'WindowGroup'})
                overlay_windows(ax, dff, F.shape[1])
        plt.tight_layout()
        if save_path:
...
class DIAFrameRenderer(BaseFrameRenderer):
    """
    Renderer for DIA-PASEF (.d via TimsDatasetDIA).
    """
    def __init__(self, data_path: str,
                 memory_limit_gb: float = 4.0,
                 use_bruker_sdk: bool = True,
                 in_memory: bool = False):
        super().__init__(memory_limit_gb)
        self.handle = TimsDatasetDIA(data_path,
                                     use_bruker_sdk=use_bruker_sdk,
                                     in_memory=in_memory)
        self.windows = self.handle.dia_ms_ms_windows.copy()
        self.frame_to_group = dict(zip(
            self.handle.dia_ms_ms_info.Frame,
            self.handle.dia_ms_ms_info.WindowGroup
        ))
        ...
        self.fragment_frames = set(
            self.handle.meta_data[self.handle.meta_data.MsMsType == 1].Id
        )
    ...
    def _render_frame(self, frame_id: int, save_path=None,
        ...
        if annotate:
            if frame_id in self.precursor_frames:
                ...
                df_w['Frame'] = frame_id
            overlay_windows(ax, df_w, F.shape[1],
                            text_dx=-20, text_dy=-20,
                            ha='right', va='bottom')
        plt.tight_layout()
        if save_path:
            os.makedirs(os.path.dirname(save_path), exist_ok=True)
