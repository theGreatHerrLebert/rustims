...
def create_chromatography_panel(state: "SimulationState") -> None:
    ...
    with ui.column().classes("w-full gap-4 p-4"):
        ...
        select_field(
            ...
            value=state.config.retention_time.koina_rt_model or "Default (GRU)",
            options=["Default (GRU)", "Deeplc_hela_hf"],
            on_change=lambda v: setattr(
                state.config.retention_time,
                "koina_rt_model",
                "" if v == "Default (GRU)" else v,
            ),
            tooltip="Deep learning model for retention time prediction",
        )
        ...
        ui.label("Ion Mobility").classes("text-lg font-bold mt-4")
...
def create_ions_panel(state: "SimulationState") -> None:
    ...
    with ui.column().classes("w-full gap-4 p-4"):
        ...
        select_field(
            ...
            value=state.config.fragment_intensity.fragment_intensity_model or "Default (Prosit)",
            options=["Default (Prosit)", "prosit", "peptdeep"],
            on_change=lambda v: setattr(
                state.config.fragment_intensity,
                "fragment_intensity_model",
                "" if v == "Default (Prosit)" else v,
            ),
        )
...
### Fragment Intensity
- **Model**: Prosit or PeptDeep for fragment ion intensity prediction
