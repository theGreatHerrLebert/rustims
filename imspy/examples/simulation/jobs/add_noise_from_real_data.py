...
def get_fragment_noise(handle, target_wg, sample_fraction: float = 0.5, max_intensity: float = 30) -> TimsFrame:
...
def add_real_data_noise_to_frames(
    ...
    for frame in frames:
        if frame.frame_id in fragment_frames:
            ...
            noise = get_fragment_noise(acquisition_builder.tdf_writer.helper_handle, target_wg, sample_fraction,
                                       intensity_max)
