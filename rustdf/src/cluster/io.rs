// src/cluster/io.rs
use super::cluster_eval::ClusterResult;
...
impl ClusterFile {
    pub fn new(clusters: Vec<ClusterResult>) -> Self {
        Self { version: 1, clusters }
    }
}

// --- JSON (human-readable) ---
pub fn save_json(path: &str, clusters: &[ClusterResult]) -> std::io::Result<()> {
...
pub fn load_json(path: &str) -> std::io::Result<Vec<ClusterResult>> {
...
pub fn save_bincode(path: &str, clusters: &[ClusterResult], compress: bool) -> std::io::Result<()> {
...
pub fn load_bincode(path: &str) -> std::io::Result<Vec<ClusterResult>> {
...
pub fn strip_heavy(mut clusters: Vec<ClusterResult>, keep_points: bool, keep_axes: bool) -> Vec<ClusterResult> {
    for c in &mut clusters {
        if !keep_points { c.raw_points = None; }
        if !keep_axes { c.frames_axis = None; c.scans_axis = None; c.mz_axis = None; }
    }
    clusters
}