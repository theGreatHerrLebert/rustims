...
impl TimsTofSyntheticsPrecursorFrameBuilder {
    ...
    pub fn build_precursor_frame(&self, frame_id: u32, mz_noise_precursor: bool, precursor_noise_ppm: f64, right_drag: bool) -> TimsFrame {
    ...
    ///
    pub fn build_precursor_frames(&self, frame_ids: Vec<u32>, mz_noise_precursor: bool, precursor_noise_ppm: f64, right_drag: bool, num_threads: usize) -> Vec<TimsFrame> {
        let thread_pool = ThreadPoolBuilder::new().num_threads(num_threads).build().unwrap();
        let mut tims_frames: Vec<TimsFrame> = Vec::new();

        thread_pool.install(|| {
            tims_frames = frame_ids.par_iter().map(|frame_id| self.build_precursor_frame(*frame_id, mz_noise_precursor, precursor_noise_ppm, right_drag)).collect();
        });
